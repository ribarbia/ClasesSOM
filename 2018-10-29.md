...
```PowerShell
New-item -Itemtype Directory Usuario1
```
- Crear un directorio para cada usuario
```PowerShell
New-Item -Path Usuario1 -Name fichereo.txt
```
- Crear un fichero para cada usuario
```PowerShell
New-Item -Path Usiario1 -Name fichero.txt
```
- Añadir contenido a cada fichero creado para cada usuario
```PowerShell
New-Item -Path Usiario1 -Name fichero.txt -ItemType file -Value
```
